...
class MockDstackClient:
    def get_key(self, path, purpose):
        # Returns an object with a .key attribute (hex string)
        mock_key = MagicMock()
        mock_key.key = "00" * 32
        return mock_key
    ...
    def get_quote(self, report_data):
        # Returns an object with a .quote attribute. 
        # We append the report_data to simulate hardware binding.
    ...
    def info(self):
        # Returns an object with an .app_id attribute
        mock_info = MagicMock()
        mock_info.app_id = "simulator_app_id"
        return mock_info


# --- 2. Patch BEFORE importing the app ---
with patch('dstack_sdk.DstackClient', new=MockDstackClient):
...
def test_audit_non_compliant_scenario():
    ...
    # Ensure the remediation report exists (Categorical Redaction)
...
def test_stateless_immutability():
    ...
    Since we removed the ledger, sending the exact same payload twice should 
    succeed but yield the exact same mathematically proven result.
    """
    handshake = client.get("/handshake").json()
